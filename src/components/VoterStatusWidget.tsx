...
export default function VoterStatusWidget({ votersStatus, title = 'Postęp wyboru w grupie', compact = false }: VoterStatusWidgetProps) {
  ...
  return (
    <div className={`bg-steam-card/90 border border-steam-border/70 rounded-3xl shadow-xl space-y-3.5 ${compact ? 'p-4' : 'p-5'}`}>
      {/* Header */}
      ...
      <div className="flex flex-wrap items-center gap-2.5">
        {votersStatus.map((v) => (
          <div
            ...
            title={`${v.personaName}: ${v.hasVoted ? 'Zapisane preferencje ✔️' : 'W trakcie wyboru ⏳'}`}
          >
